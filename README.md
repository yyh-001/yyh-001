<!--
  GitHub Profile README · yyh-001
  https://github.com/yyh-001
-->

<p align="center">
  <img src="./assets/banner.png" alt="yyh — Agent Skills & LLM tools" width="100%" />
</p>

<div align="center">

# Hey, I'm **yyh** 👋

**做 Agent 真正能用的工具** — Skill、斗图、人设、模型性价比，从「能跑」做到「想用」。

[![GitHub followers](https://img.shields.io/github/followers/yyh-001?style=for-the-badge&logo=github&color=181717)](https://github.com/yyh-001)
[![LLM Rankings](https://img.shields.io/badge/LLM_Rankings-Live-00C853?style=for-the-badge&logo=googlechrome&logoColor=white)](https://yyh-001.github.io/llm-value-rankings/)
[![QQ Group](https://img.shields.io/badge/QQ_群-993579665-12B7F5?style=for-the-badge&logo=tencentqq&logoColor=white)](https://qm.qq.com/q/7AD2g70HqS)

<br/>

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=3200&pause=900&color=58A6FF&center=true&vCenter=true&multiline=true&width=680&height=70&lines=Agent+Skills+%C2%B7+OpenClaw+%C2%B7+Hermes;LLM+Value+Rankings+%C2%B7+300%2B+models;Make+agents+feel+human" alt="typing" />

</div>

---

## ⚡ Now Shipping

<table>
<tr>
<td width="50%" valign="top">

### 🏆 [llm-value-rankings](https://github.com/yyh-001/llm-value-rankings)
**大模型性价比排行榜** · [在线看 →](https://yyh-001.github.io/llm-value-rankings/)

每天更新 300+ 模型：智力 · 速度 · 价格，一眼看出谁最值。

```text
OpenRouter × Artificial Analysis → 静态站
```

</td>
<td width="50%" valign="top">

### 🎭 [agent-expression](https://github.com/yyh-001/agent-expression)
**本地表情包 Skill**

按心情搜图、识图入库，只返回真实绝对路径。  
适配 OpenClaw / Hermes / Cursor。

```bash
curl -fsSL .../install.sh | bash
```

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 💬 [suki](https://github.com/yyh-001/suki)
**Suki（小琦）人格 Skill**

元气会接梗，去客服腔、去 AI 味。  
和表情包 Skill 独立搭配，聊成损友而不是客服。

</td>
<td width="50%" valign="top">

### 📝 [exam](https://github.com/yyh-001/exam)
**在线考试 Web** · [演示 →](http://exam.yyhnet.cn)

基于 uni-app 的考试前端，早期作品，仍在线上。

</td>
</tr>
</table>

<p align="center">
  <a href="https://github.com/yyh-001/llm-value-rankings">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=yyh-001&repo=llm-value-rankings&theme=tokyonight&hide_border=true&bg_color=0D1117" alt="llm-value-rankings" />
  </a>
  <a href="https://github.com/yyh-001/agent-expression">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=yyh-001&repo=agent-expression&theme=tokyonight&hide_border=true&bg_color=0D1117" alt="agent-expression" />
  </a>
</p>
<p align="center">
  <a href="https://github.com/yyh-001/suki">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=yyh-001&repo=suki&theme=tokyonight&hide_border=true&bg_color=0D1117" alt="suki" />
  </a>
  <a href="https://github.com/yyh-001/exam">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=yyh-001&repo=exam&theme=tokyonight&hide_border=true&bg_color=0D1117" alt="exam" />
  </a>
</p>

---

## 🧠 What I Care About

```text
┌─────────────────────────────────────────────────────────┐
│  Agent 不该只会「说」——要能搜图、换人设、算性价比     │
│                                                         │
│  Skill 优先 · 主机无关 · 真实路径 · 少 AI 味            │
│  OpenClaw / Hermes / Cursor 都能接，不绑死一家          │
└─────────────────────────────────────────────────────────┘
```

| 方向 | 在做什么 |
|------|----------|
| **Agent Skills** | 表情包管线、人格、可安装的本地能力 |
| **LLM 工具** | 模型对比、定价与智力的可读排行 |
| **Bot / IM** | 聊天搭子、多端 Agent 接入 |

---

## 🛠️ Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,ts,js,vue,react,nodejs,docker,linux,git,github,css,html" alt="stack" />
</p>

<p align="center">
  <code>Python</code> · <code>TypeScript</code> · <code>Agent Skills</code> · <code>OpenClaw</code> · <code>Hermes</code> · <code>LLM APIs</code>
</p>

---

## 📊 Stats

<div align="center">

<img height="180" src="https://github-readme-stats.vercel.app/api?username=yyh-001&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&count_private=true&include_all_commits=true" alt="stats" />
<img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=yyh-001&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&langs_count=8" alt="langs" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=yyh-001&theme=tokyonight&hide_border=true&background=0D1117" alt="streak" />

<br/>

<img src="https://github-profile-trophy.vercel.app/?username=yyh-001&theme=tokyonight&no-frame=true&row=1&column=7&margin-w=8" alt="trophy" />

</div>

---

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/yyh-001/yyh-001/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/yyh-001/yyh-001/output/github-contribution-grid-snake.svg" />
  <img alt="github contribution snake" src="https://raw.githubusercontent.com/yyh-001/yyh-001/output/github-contribution-grid-snake.svg" />
</picture>

</div>

---

## 🐍 Contribution Snake

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/yyh-001/yyh-001/output/github-contribution-grid-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/yyh-001/yyh-001/output/github-contribution-grid-snake.svg" />
  <img alt="github contribution snake" src="https://raw.githubusercontent.com/yyh-001/yyh-001/output/github-contribution-grid-snake.svg" />
</picture>

</div>

---

<div align="center">

### 找我玩 / 反馈

**QQ 群** [993579665](https://qm.qq.com/q/7AD2g70HqS) · 仓库 Issue 也行

<br/>

<img src="https://komarev.com/ghpvc/?username=yyh-001&style=for-the-badge&color=58A6FF" alt="profile views" />

<br/>

*"Make agents useful — and a little more human."*

</div>
